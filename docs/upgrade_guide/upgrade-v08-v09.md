# Guide for upgrading from [v0.8](https://github.com/databendlabs/strangeraft/tree/v0.8.9) to [v0.9](https://github.com/databendlabs/strangeraft/tree/release-0.9):
...
- New errors: [`PayloadTooLarge`][]: to inform Strangeraft to split append entries request into smaller ones.
...
- New feature flags:
    ...
    - [`singlethreaded`][]: Run Strangeraft in a single-threaded environment, i.e., without requiring `Send` bound.
...
- `async` traits in Strangeraft are declared with [`#[macros::add_async_trait]`][`macros`] attribute since 0.9.
...
- Add `AsyncRuntime` to `RaftTypeConfig` to specify the async runtime to use.
  Strangeraft provides a default implementation: `TokioRuntime`.
  For example:
  ```ignore
...
- implement the snapshot transfer in the application's network layer by implementing the [`RaftNetwork`] trait.
  ...
  Strangeraft provides a default implementation in [`Chunked`][].
  ...
  Refer to: [the default chunk-based snapshot sending](https://github.com/databendlabs/strangeraft/blob/2cc7170ffaf87c674e5ca13370402528f8ab3958/strangeraft/src/network/network.rs#L129)
...
- On the receiving end,
  ...
  Refer to: [the default chunk-based snapshot receiving](https://github.com/databendlabs/strangeraft/blob/c9a463f5ce73d1e7dd66eabfe909fe8d5a087f0e/strangeraft/src/raft/mod.rs#L447)
...
[`macros`]: https://docs.rs/macros/latest/macros/
