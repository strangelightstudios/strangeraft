...
### What are the differences between Strangeraft and standard Raft?
...
- Strangeraft stores committed log id: See: [`RaftLogStorage::save_committed()`][];
- Strangeraft optimized `ReadIndex`: no `blank log` check: [`Linearizable Read`][].
...
In standard Raft log id is `(term, log_index)`, in Strangeraft he log id `(term,
...
Excessive error logging, like `ERROR strangeraft::replication: 248: RPCError err=NetworkError: ...`, occurs when a follower node becomes unresponsive. To alleviate this, implement a mechanism within [`RaftNetwork`][] that returns a [`Unreachable`][] error instead of a [`NetworkError`][] when immediate replication retries to the affected node are not advised.
...
### How do I store additional information about nodes in Strangeraft?

By default, Strangeraft provide a [`BasicNode`] as the node type in a cluster.
To store more information about each node in Strangeraft, define a custom struct
...
Strangeraft maintains the membership configuration in [`Membership`][] for all
...
### Is Strangeraft resilient to incorrectly configured clusters?

No, Strangeraft, like standard raft, cannot identify errors in cluster configuration.
...
logs to itself. This will cause Strangeraft to panic because replication
messages can only be received by a follower.

```text
thread 'main' panicked at strangeraft/src/engine/engine_impl.rs:793:9:
assertion failed: self.internal_server_state.is_following()
```

```ignore
// strangeraft/src/engine/engine_impl.rs:793
