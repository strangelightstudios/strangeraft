# Getting Started with Strangeraft

In this chapter, we will build a key-value store cluster using Strangeraft.

1. [examples/raft-kv-memstore](https://github.com/databendlabs/strangeraft/tree/main/examples/raft-kv-memstore)
...
1. [examples/raft-kv-rocksdb](https://github.com/databendlabs/strangeraft/tree/main/examples/raft-kv-rocksdb)
...
store cluster with Strangeraft.
...
Implementing your own Raft-based application with Strangeraft is quite simple, and it involves:
...
Strangeraft is a generic implementation of Raft. It requires the application to define
...
impl strangeraft::RaftTypeConfig for TypeConfig {
    type D            = Request;
    type R            = Response;

    // Following are absent in `declare_raft_types` and filled with default values:
    type NodeId       = u64;
    type Node         = strangeraft::impls::BasicNode;
    type Entry        = strangeraft::impls::Entry<TypeConfig>;
    type Responder    = strangeraft::impls::OneshotResponder<TypeConfig>,
    type AsyncRuntime = strangeraft::impls::TokioRuntime;
...
Strangeraft provides default implementations for mostly used types:
...
[`Mem KV Store`](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/src/store/mod.rs),
...
There is a [Test suite for RaftLogStorage and RaftStateMachine][`LogSuite`] available in Strangeraft.
If your implementation passes the tests, Strangeraft should work well with it.
To test your implementation, run `Suite::test_all()` with a [`StoreBuilder`] implementation,
as shown in the [`RocksStore` test](https://github.com/databendlabs/strangeraft/blob/main/examples/rocksstore/src/test.rs).
...
[Mem KV Network](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/src/network/raft_network_impl.rs)
...
[Mem KV Server](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/src/network/raft.rs).
...
> because Strangeraft provides a blanket implementation that adapts `RaftNetwork`
> implementations to `RaftNetworkV2`. For example:
>
> ```rust,ignore
> pub trait RaftTypeConfigExt: strangeraft::RaftTypeConfig {}
...
> conflicting implementation in crate `strangeraft`:
...
In Strangeraft, an implementation of [`RaftNetwork`] needs to connect to remote Raft peers. To store additional information about each peer, you need to specify the `Node` type in `RaftTypeConfig`:
...
Then use `Raft::add_learner(node_id, BasicNode::new("127.0.0.1"), ...)` to instruct Strangeraft to store node information in [`Membership`]. This information is then consistently replicated across all nodes, and will be passed to [`RaftNetworkFactory::new_client()`] to connect to remote Raft peers:
...
[main.rs](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/src/lib.rs)
:
...
The [examples/raft-kv-memstore](https://github.com/databendlabs/strangeraft/tree/main/examples/raft-kv-memstore)
...
- [test-cluster.sh](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/test-cluster.sh)
...
- [test_cluster.rs](https://github.com/databendlabs/strangeraft/blob/main/examples/raft-kv-memstore/tests/cluster/test_cluster.rs)
