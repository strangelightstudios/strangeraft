...
Strangeraft provides two `LeaderId` types to switch between these two modes:
...
Within Strangeraft, and also implicitly in the standard Raft, `LeaderId` is used to uniquely identify a leader.
...
- Conversely, in Strangeraft, with [`leader_id_adv::LeaderId`], `LeaderId` follows a `total order` based on lexicographical comparison:
...
When handling `VoteRequest`, both Strangeraft and standard Raft (though not explicitly detailed) rely on the ordering of
...
Consequently, by default in Strangeraft (with [`leader_id_adv::LeaderId`]), it is possible to elect multiple `Leader`s
...
Use `strangeraft::impls::leader_id_adv::LeaderId` for [`RaftTypeConfig::LeaderId`] or leave it to default to switch to advanced mode.
...
Use `strangeraft::impls::leader_id_std::LeaderId` for [`RaftTypeConfig::LeaderId`] to switch to standard mode.
