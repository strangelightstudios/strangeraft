# Understanding Strangeraft Log Pointers

Inside Strangeraft, there are several pointers pointing to the raft-log:
...
- `purged`:    the last log entry that has been purged from the log, inclusive.
  ...
  strangeraft purges logs from the beginning to a configured position, when a fresh snapshot is built,
  `purged` is the greatest log id that strangeraft **does not store**.

- `snapshot`:  the last log entry that has been included in the latest snapshot, inclusive.
  Strangeraft takes a snapshot of the state-machine periodically.
...
- `accepted`: the last log entry accepted from the Leader but not yet submit to the storage, inclusive.
  `accepted` is also known as `last_log` in Strangeraft codebase.

Within strangeraft, the log pointers mentioned above always hold that:
...
Strangeraft provides optional API for application to store `committed` log id.
...
- If the `committed` log id is not saved, Strangeraft will just recover the state machine to
