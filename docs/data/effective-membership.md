...
The membership config log in Strangeraft becomes **effective** upon being
viewed, resulting in the latest log being the effective membership.

As a result, the effective membership in Strangeraft is **unstable** until it is
committed, as uncommitted logs may be overwritten by a new leader. Therefore,
Strangeraft will revert membership config to the previous membership config log,
...
In Strangeraft, the membership state contains these two configs:
