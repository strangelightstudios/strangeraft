...
In Strangeraft, the [`Vote`] defines the pseudo time which determines the `leader` in a distributed consensus.
...
The validity checking for RPCs in Strangeraft, such as when handling vote or append-entries requests,
...
Note: follower and learner in Strangeraft are almost the same. The only difference
