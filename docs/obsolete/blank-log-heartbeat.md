...
[Issue-698](https://github.com/databendlabs/strangeraft/issues/698)
...
## Strangeraft heartbeat is a blank log
...
The **pseudo time** in strangeraft is a tuple `(vote, last_log_id)`, compared in dictionary order(`vote` is equivalent concept as round number in Paxos).
...
- Simple, get rid of a timer.
  ...
  With heartbeat log (the way strangeraft does), when handling a vote request, it only needs to examine one value: the raft time: `(last_term, last_log_index)`. This makes the logic simpler and the test easier to write.
