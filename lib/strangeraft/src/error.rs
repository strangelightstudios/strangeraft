...
/// Error that indicates a **temporary** network error and when it is returned, Strangeraft will retry
/// immediately.
///
...
/// Upon encountering this error, Strangeraft will invoke [`backoff()`] to implement a delay before
...
/// backoff period, whereas with [`NetworkError`], Strangeraft may attempt an immediate retry.
///
...
/// A [`RaftNetwork`] implementation returns this error to inform Strangeraft to divide an
/// [`AppendEntriesRequest`] into smaller chunks.
/// Strangeraft will immediately retry sending in smaller chunks.
/// If the request cannot be divided (contains only one entry), Strangeraft interprets it as
/// [`Unreachable`].
///
/// A hint can be provided to help Strangeraft in splitting the request.
///
