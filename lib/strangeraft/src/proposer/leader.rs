...
/// Leading state data.
///
/// Strangeraft leading state is the combination of Leader and Candidate in original raft.
...
///
/// By combining candidate and leader into one stage, strangeraft does not need to lose leadership when
    ...
    QS: QuorumSet<C::NodeId> + Clone + fmt::Debug + 'static,
{
    ...
    pub(crate) fn last_quorum_acked_time(&mut self) -> Option<InstantOf<C>> {
        ...
        // Safe unwrap: voted_for() is always non-None in Strangeraft
