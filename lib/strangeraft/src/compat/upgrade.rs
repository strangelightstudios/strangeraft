...
///
/// This trait is used to define types that can be directly upgraded from older versions of strangeraft
/// to newer versions. For example, `LogId` can be upgraded: in strangeraft 0.7, `LogId` is `(term,
/// index)`, which is upgraded to `(CommittedLeaderId, index)` in strangeraft 0.8.
