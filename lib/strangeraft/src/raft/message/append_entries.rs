...
///
/// In Strangeraft a heartbeat [`AppendEntriesRequest`] message could have `prev_log_id=None` and
...
pub enum AppendEntriesResponse<C: RaftTypeConfig> {
    ...
    /// Strangeraft that the first log id([`AppendEntriesRequest::prev_log_id`]) may not match on
