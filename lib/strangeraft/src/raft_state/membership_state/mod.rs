...
where C: RaftTypeConfig
{
    ...
    pub(crate) fn append(&mut self, m: Arc<EffectiveMembership<C>>) {
        ...
        // Strangeraft allows at most only one non-committed membership config.
    ...
    ///
    /// In strangeraft the last seen membership config, whether committed or not, is the effective
    /// one.
    ///
    /// A committed membership config may or may not be the same as the effective one.
